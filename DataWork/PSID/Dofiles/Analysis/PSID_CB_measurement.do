	...
	*	Make family-level data
		...
		*	Keep relevant family-level variables only
		...
		loc	demovars		age_head*	race_head*	total_income_fam*	marital_status*	gender_head_fam*	state_resid_fam*	
		loc	familyvars		num_FU_fam*	num_child_fam*	family_comp_change*	child_in_FU*	fam_comp_nochange_99_03	fam_comp_samehead_99_03
		loc	socioeconvars	edu_years_head_fam*	FPL*	grade_comp*
		loc	foodvars		fs_raw*	fs_scale*	fs_cat*	food_stamp_used*	child_meal_assist*	WIC_received*
		
		keep	`interview_ID'	`weightvars'	`samplingvars'	`demovars'	`familyvars'	`socioeconvars'	`foodvars'
	...
	*	Before re-shaping data, we need to decide what families to track (so we have only 1 obs per family in baseyear)
		...
		*	Families with no member change during 1999-2003
		keep	if	fam_comp_nochange_99_03==1
	...
	*	Declare list of variables
	local	demovars	c.age_head_fam##c.age_head_fam i.race_head_cat gender_head_fam	i.marital_status_cat	edu_years_head_fam
	local	econvars	total_income_fam_wins
	local	famvars		num_FU_fam	num_child_fam
	local	foodvars	food_stamp_used*	child_meal_assist	WIC_received_last*
