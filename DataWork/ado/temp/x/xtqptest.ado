...
** 25sep2017: Minor bug fix. No longer breaks when panel contains gaps.
		...
		*** Q(p) test ("upto order")
		if "`test'" == "Q(p)" & "`tooShort'" == "" {
			if "`balance'" == "balanced" mata: qp_statistic("`var'", "`toUse2'", `timelength', `panelunits', `p')
				...
				scalar pvalue = .
				scalar `pvalue' = .
				local errorMessage "One of the panels contains gaps, Q(P) is not defined in this context. Try xtistest (large N, small T) or xtserial."
			}
